...
#
# - check that the ipython file (``run_pre_spinup.ipynb``, ``run_simulation.ipynb``, ``run_benchmarking.ipynb``) is synchronized with its python file
...
sim_date = "2023-07-25" # publication
...
pre_spinup_data = [
    {
        "filename": "run_pre_spinup",
        "params": [pre_spinup_date]
    },
]
...
four_scenarios_data = [
    {
        ...
        "params": [
            pre_spinup_date, cc_spinup_species, str(cc_spinup_length), str(cc_spinup_N),
            sim_date, sim_name, str(sim_N), emergency_action_str, emergency_direction, emergency_stand_action_str
        ]
    }
...
benchmarking_speciess = ["pine"]
benchmarking_data = [
    {
        "filename": "run_benchmarking",
        "params": [pre_spinup_date, sim_date, species]
    }
...
mixed_aged_data = [
    {
       "filename": "run_simulation",
       "params": [
            pre_spinup_date, cc_spinup_species, str(cc_spinup_length), str(cc_spinup_N_),
            sim_date, sim_name, str(sim_N), emergency_action_str, emergency_direction, emergency_stand_action_str
        ]
    }
    for sim_name in ["mixed-aged_pine_long"] for cc_spinup_N_ in [1000, 1200, 2000]
]
# -

notebook_datas = pre_spinup_data + four_scenarios_data + benchmarking_data# + mixed_aged_data
notebook_datas
