"""
This module contains the abstract base class :class:`~.soil_c_model_abc.SoilCModelABC`.

All carbon cycle soil model classes should derive from here in order
...
class SoilCModelABC(metaclass=ABCMeta):
    """
    Abstract base class for all carbon cycle soil models.
    ...
    def __init__(
        ...
        # create start_values !!! arbitrary !!!
        sv = np.zeros(self.srm.nr_pools)
        ...
        for (pool_from, pool_to), expr in self.srm.internal_fluxes.items():
            internal_flux_funcs[
                (pool_from, pool_to)
            ] = numerical_function_from_expression(
    ...
    def update(self, input_fluxes: InputFluxes) -> OutputFluxesInt:
        """Update the litter and soil pools.

        Args:
            input_fluxes: dictionary of fluxes into the soil model

            - pool_name: flux

        Returns:
            soil's output fluxes dictionary
