# -*- coding: utf-8 -*-
"""
General methods related to vegetation biomass amount and its variation among sites.
"""
...
def naslund_heightcurve(d, species, p=None):
    ...
    """
    para = {
        "pine": [2.0, 1.181, 0.247],
        "spruce": [3.0, 1.703, 0.327],
        "birch": [2.0, 1.014, 0.238],
    }
    if p is None:
        p = para[species]
...
def profiles_hyde(data, species, z):
    ...
    Returns:
        ...
        mleaf - leaf mass (kg)
        N - trees ha-1
        lad - leaf-area density (m2m-1), integrates to 1 * L[k]
    """
    d = data[:, 0]  # dbh, cm
    N = data[:, 1]  # trees ha-1
...
#        y3=np.exp(10.2806*(d/(d+10))-3.3633)    #living branches excluding leaves
...
def crown_biomass_distr(species, z, htop=1, hbase=0, PlotFigs=False):
    ...
    # TODO: that does not make much sense, a mean only of the previous value?
    Ldens[ind] = np.mean(Ldens[ind - 1 : ind])
    #    Ldens[ind]=np.mean(Ldens[ind-1:ind]+1) # Holger: my suggestion
    ...
    Cdens[ind] = np.mean(Ldens[ind - 1 : ind + 1])  # Holger: my suggestion
...
def understory_biomass(site, age, x=[]):
    ...
    print(
        "Vegetation.understory_biomass: computing sitetype "
        + site
        + " using "
        + model
        + " -based model"
    ...
    if site.lower() == "pine_upland":
        ...
        if np.size(x) == 0:  # only age as explaning variable
            ...
            y[3] = (
                -53.196 + 0.378 * x[4] * x[0] - 0.014 * x[4] * x[3] + 2e-5 * x[3] ** 2
            )  # lichens
            y[4] = (
                13.865 + 0.013 * x[0] * x[1] - 2.969 * x[4] + 3e-5 * x[3] * age
            )  # Field layer total
            y[5] = (
                8.623
                + 0.09 * x[4] * x[0]
    ...
    if site.lower() == "spruce_upland":
        ...
        if np.size(x) == 0:  # only age as explaning variable
            # dependent variable is sqrt(biomass -0.5)
            y[0] = (
                10.375 - 0.033 * age + 0.001 * age**2 - 6e-6 * age**3
            )  # Dwarf shrubs
            ...
            y[0] = (
                10.903 + 0.027 * x[4] * age + 0.045 * x[4] * x[0] - 6e-6 * x[3] ** 2
            )  # Dwarf shrubs
            y[1] = (
                21.49 - 0.05 * x[4] * x[0] - 0.006 * x[4] * x[5] - 0.008 * x[4] * age
            )  # Herbs&grasses
            y[2] = (
                9.672 + 0.029 * x[4] * age + 0.078 * x[4] * x[0] + 0.186 * x[7]
            )  # mosses
            y[3] = (
                -42.593 + 0.981 * x[0] - 0.008 * x[7] ** 2 + 0.002 * x[7] * age
            )  # Field layer total
            y[4] = (
                22.522 + 0.026 * x[4] * age + 0.11 * x[4] * x[0] - 0.003 * x[4] * x[3]
            )  # Total
    ...
    if site.lower() == "broadleaved_upland":
        ...
        if np.size(x) == 0:  # only age as explaning variable
            ...
            y[1] = (
                20.58 - 0.423 * age + 3e-4 * age**2 - 2e-5 * age**3
            )  # Herbs&grasses
...
def taper_curve(h, d, species, L=1.3, dz=0.01):
    """
    Returns taper curve (bole shape) based on Laasasenaho, 1982 eq. 33.1 [parameters 41.1]
    Args:
        h - tree height [m]
        d - diameter [cm] at height L [m]
        species - 'pine', 'spruce', 'birch'
        L - height of d [m]
        dz - height increments [m]
    Returns:
        hs, dh - height and diameter arrays
    """
    params = {
        "pine": [2.2188, -0.63157, -1.6082, 2.4886, -2.4147, 2.3619, -1.7539, 1.0817],
        "spruce": [2.3366, -3.2684, 3.6513, -2.2608, 0.0, 2.1501, -2.7412, 1.8876],
        "birch": [0.93838, 4.1060, -7.8517, 7.8993, -7.5018, 6.3863, -4.3918, 2.1604],
    }
    p = params[species]

    hs = np.arange(0.05, h, dz)
    # compute reference diameter (at 0.2h)
    x = 1 - L / h
    rd0 = (
        p[0] * x
        + p[1] * x**2
    ...
    d0 = d / rd0
    # compute diameter at different heights
    x = 1 - hs / h
    rd = (
        p[0] * x
        + p[1] * x**2
