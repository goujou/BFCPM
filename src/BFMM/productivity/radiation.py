# -*- coding: utf-8 -*-
"""Distribution of within canopy radiation."""
...
from .constants import (DEG_TO_KELVIN, DEG_TO_RAD, EPS, SPECIFIC_HEAT_AIR,
                        STEFAN_BOLTZMANN)
...
def solar_angles(lat, lon, jday, timezone=+2.0):
    ...
    decl = (
        ...
        - 0.399912 * np.cos(y)
        + 7.0257e-2 * np.sin(y)
        - 6.758e-3 * np.cos(2.0 * y)
        + 9.07e-4 * np.sin(2.0 * y)
        - 2.697e-3 * np.cos(3.0 * y)
        + 1.48e-3 * np.sin(3.0 * y)
    )
    ...
    et = 229.18 * (
        ...
        + 1.868e-3 * np.cos(y)
        - 3.2077e-2 * np.sin(y)
        - 1.4615e-2 * np.cos(2.0 * y)
        - 4.0849e-2 * np.sin(2.0 * y)
    )
    # print et / 60.
    # hour angle
    offset = et + 4.0 * lon - 60.0 * timezone
    fday = np.modf(jday)[0]  # decimal day
    ha = DEG_TO_RAD * ((1440.0 * fday + offset) / 4.0 - 180.0)  # rad
    ...
    sunrise = 720.0 - 4.0 * (lon + ha0) - et  # minutes
    sunset = 720.0 - 4.0 * (lon - ha0) - et  # minutes
...
# def freeze_ZhaoQualls(photop, Qp, *remaining_args, **kwargs):
#    # lru, make A_gs' parameters immutable
#    _photop = frozendict({
#        key: (
#            val if not isinstance(val, dict) else
#            frozendict({sub_key: tuple(sub_val) for sub_key, sub_val in val.items()})
...
#
#
# def unfreeze_ZhaoQualls(_LAIz, *remaining_args, **kwargs):
#    LAIz = np.array(_LAIz)
#
#    return (LAIz,) + remaining_args, kwargs
#
# @make_lru_cacheable(unfreeze_func=unfreeze_ZhaoQualls, maxsize=50_000)
# makes no hits
def canopy_sw_ZhaoQualls(
    LAIz, Clump, x, ZEN, IbSky, IdSky, LeafAlbedo, SoilAlbedo, PlotFigs=False
):
    """
    Computes incident (Wm-2 ground) SW radiation and absorbed (Wm-2 (leaf) radiation within canopies.
    INPUT:
        LAIz: layerwise one-sided leaf-area index (m2m-2)
    ...
    for k in range(1, M + 1):
        ...
        A[2 * k - 1, 2 * k] = 1 - rd[k - 1] * rd[k] * (1 - aL[k - 1]) * (
            1 - taud[k - 1]
        ) * (1 - aL[k]) * (1 - taud[k])
    ...
    # Samuli's original formulation
    Q_sh = Kd * (SWdo + SWuo)  # normal to shaded leaves is all diffuse
    Q_sl = Kb * IbSky + Q_sh  # normal to sunlit leaves is direct and diffuse

    # Kersti's change to be consistent with q_sh
    # Q_sh = Clump * Kd*(SWdo + SWuo)  # normal to shaded leaves is all diffuse. multiply by Clump as clumping means elements shade each other
    # Q_sl = Kb*IbSky + Q_sh  # normal to sunlit leaves is direct and diffuse
    ...
    if PlotFigs:
        ...
        plt.legend(
            (
                "f_{sl}, total = %.2f" % np.sum(f_slo * LAIz),
                "f_{sh}, total = %.2f" % np.sum((1 - f_slo) * LAIz),
            ),
            loc="best",
        )
        ...
        plt.plot(
            (1 - np.exp(-Kd * Lo)) * (SWdo + SWuo) / (LAIz + EPS), -Lcumo / Clump, "-k"
        )
        plt.ylabel("-Lcum eff.")
...
def canopy_sw_Spitters(
    ...
    INPUT:
        ...
        x: param. of leaf angle distribution
            (1=spherical, 0=vertical, inf.=horizontal) (-)
        ZEN: solar zenith angle (rad), scalar
        IbSky: incident beam radiation above canopy (Wm-2),scalar # in our case umol m-2
        IdSky: downwelling diffuse radiation above canopy (Wm-2),scalar # in our case umol m-2
        LAI: leaf (or plant-area) index, 1-sided (m2m-2),scalar
        LeafAlbedo: leaf albedo of desired waveband (-)
        SoilAlbedo: soil albedo of desired waveband (-)
        PlotFigs="True" plots figures.
    ...
    OUTPUT:
        ...
        Q_sl: incident SW normal to sunlit leaves (Wm-2)
        Q_sh: incident SW normal to shaded leaves (Wm-2)
        q_sl: absorbed SW by sunlit leaves (Wm-2(leaf)) # in our case umol m-2
        q_sh: absorbed SW by shaded leaves (Wm-2(leaf)) # in our case umo m-2
        q_soil: absorbed SW by soil surface (Wm-2(ground))
    ...
    AA = ((rb1 - SoilAlbedo) / (rb1 * SoilAlbedo - 1.0)) * np.exp(
        -2.0 * (1.0 - LeafAlbedo) ** 0.5 * Kb * LAI
    )
    rb1 = (rb1 + AA) / (1.0 + rb1 * AA)  # beam
    del AA

    AA = ((rd1 - SoilAlbedo) / (rd1 * SoilAlbedo - 1.0)) * np.exp(
        -2.0 * (1.0 - LeafAlbedo) ** 0.5 * Kd * LAI
    )
    ...
    qd1 = (
        (1.0 - rd1) * IdSky * np.exp(-((1.0 - LeafAlbedo) ** 0.5) * Kd * Lcum)
    )  # attenuated diffuse
    qb1 = IbSky * np.exp(-Kb * Lcum)  # beam
    qbt1 = (
        (1.0 - rb1) * IbSky * np.exp(-((1.0 - LeafAlbedo) ** 0.5) * Kb * Lcum)
    )  # total beam
    ...
    Ad1 = (
        ...
        * (1.0 - LeafAlbedo) ** 0.5
        * Kd
        * np.exp(-((1.0 - LeafAlbedo) ** 0.5) * Kd * Lcum)
    )
    Abt1 = (
        (1.0 - rb1)
        * IbSky
    ...
    if PlotFigs:
        ...
        plt.legend(
            (
                "f_{sl}, total = %.2f" % np.sum(f_sl * LAIz),
                "f_{sh}, total = %.2f" % np.sum((1 - f_sl) * LAIz),
            ),
            loc="best",
        )
...
def compute_clouds_rad(doy, zen, Rg, H2O, Tair):
    ...
    Qclear = np.maximum(
        ...
        (
            So
            * (1.0 + 0.033 * np.cos(2.0 * np.pi * (np.minimum(doy, 365) - 10) / 365))
            * np.cos(zen)
        ),
    )
    ...
    emi0 = 1.24 * (ea / (Tair + 273.15)) ** (1.0 / 7.0)  # Song et al 2009

    # all-sky emissivity (cloud-corrections)
    #    emi_sky = (1.0 + 0.22 * f_cloud**2.75) * emi0  # Maykut & Church (1973)
    emi_sky = (
        1 - 0.84 * f_cloud
    ) * emi0 + 0.84 * f_cloud  # Song et al 2009 / (Unsworth & Monteith, 1975)
...
def canopy_lw(LAIz, Clump, x, T, LWdn0, LWup0, leaf_emi=1.0, PlotFigs=False):
    ...
    for k in range(cantop, -1, -1):
        LWdn[k] = tau[k] * LWdn[k + 1] + (1 - tau[k]) * (
            leaf_emi * STEFAN_BOLTZMANN * (T[k] + DEG_TO_KELVIN) ** 4
        )
    del k
    ...
    for k in range(1, cantop + 2):
        LWup[k] = tau[k - 1] * LWup[k - 1] + (1 - tau[k - 1]) * (
            leaf_emi * STEFAN_BOLTZMANN * (T[k - 1] + DEG_TO_KELVIN) ** 4
        )
    del k
    ...
    LWleaf[0 : cantop + 1] = (
        ...
        * (
            LWdn[1 : cantop + 2]
            + LWup[0 : cantop + 1]
            - 2 * STEFAN_BOLTZMANN * leaf_emi * (T[0 : cantop + 1] + DEG_TO_KELVIN) ** 4
        )
        / (LAIz[0 : cantop + 1] + EPS)
    )
    LWnet[0 : cantop + 1] = (
...
def canopy_lw_ZhaoQualls(
    LAIz, Clump, x, Tleaf, LWdn0, LWup0, leaf_emi=0.98, soil_emi=0.98, PlotFigs=False
):
    """
    ...
    T = np.flipud(
        np.interp(Lcum, np.flipud(Lcumo), np.flipud(Tleaf))
    ...
    A[1 : 2 * M + 1 : 2] = -(
        taud[1 : M + 1]
        + (1 - taud[1 : M + 1]) * (1 - aL[1 : M + 1]) * (1 - rd[1 : M + 1])
    )
    A[2 : 2 * M + 1 : 2] = 1 - rd[1 : M + 1] * rd[2 : M + 2] * (1 - aL[1 : M + 1]) * (
        1 - taud[1 : M + 1]
    ...
    B[2 * M + 1] = 1.0
    # superdiagonal
    C[1 : 2 * M + 1 : 2] = 1 - rd[0:M] * rd[1 : M + 1] * (1 - aL[0:M]) * (
        1 - taud[0:M]
    ) * (1 - aL[1 : M + 1]) * (1 - taud[1 : M + 1])
    C[2 : 2 * M + 1 : 2] = -(
        taud[1 : M + 1]
        + (1 - taud[1 : M + 1]) * (1 - aL[1 : M + 1]) * (1 - rd[1 : M + 1])
    )
    ...
    LWleaf[0 : cantop + 1] = (
        ...
        * (
            LWdn[1 : cantop + 2]
            + LWup[0 : cantop + 1]
            - 2 * STEFAN_BOLTZMANN * (Tleaf[0 : cantop + 1] + DEG_TO_KELVIN) ** 4
        )
        / (LAIz[0 : cantop + 1] + EPS)
    )
    ...
    if PlotFigs:
        ...
        plt.subplot(222)
        plt.plot(
            (-LWd[1:] + LWd[:-1] - LWu[:-1] + LWu[1:]) / (L[1:-1] / Clump + EPS),
            -xi[1:] / Clump,
            "go",
            label="LWnet",
        )
...
def test_radiation_functions(
    LAI,
    Clump,
    ZEN,
    x=1.0,
    method="canopy_sw_ZhaoQualls",
    LAIz=None,
    leaf_emi=0.98,
    soil_emi=0.98,
    LeafAlbedo=0.12,
    ...
    if method == "canopy_sw_ZhaoQualls":
        print(
            "------TestRun of radiation.canopy_sw_ZhaoQualls with given LAI and CLUMP -----------"
        )
        SWb, SWd, SWu, Q_sl, Q_sh, q_sl, q_sh, q_soil, f_sl, alb = canopy_sw_ZhaoQualls(
            LAIz, Clump, x, ZEN, IbSky, IdSky, LeafAlbedo, SoilAlbedo, PlotFigs="True"
        )
    ...
    if method == "canopy_sw_Spitters":
        print(
            "------TestRun of radiation.canopy_sw_Spitters with given LAI and predefined lad profile-----------"
        )
    ...
    if method == "canopy_lw_ZhaoQualls":
        print(
            "------TestRun of radiation.canopy_lw_ZhaoQualls with given LAI and CLUMP -----------"
        )
