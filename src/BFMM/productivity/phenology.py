# -*- coding: utf-8 -*-
"""
Plant phenology, seasonal cycle of photosynthetic capacity and leaf-area development.
"""
...
class Photo_cycle(object):
    ...
    def reset(self):
        # reset photo cycle model at begining of each year
        self.X = 0.0
        self.f = self.fmin
...
class LAI_cycle(object):
    ...
    def reset(self):
        # reset LAI-cycle model at begining of each year
        self.DDsum = 0.0
        self.f = self.LAImin
    ...
    def run(self, doy, T):
        ...
        # senescence phase
        if doy > self.sso:
            f = 1.0 - (1.0 - self.LAImin) * np.minimum(
                1.0, (doy - self.sso) / self.sdur
            )
...
def daylength(lat, lon, doy):
    ...
    dl = (
        2.0
        * np.arccos(cosZEN - np.sin(lat) * np.sin(decl) / (np.cos(lat) * np.cos(decl)))
        / DEG_TO_RAD
        / 15.0
    )  # hours
